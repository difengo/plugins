...
// set to call the "calc" service "metrics" endpoint
func (c *Client) BuildMetricsRequest(ctx context.Context, v interface{}) (*http.Request, error) {
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: MetricsCalcPath()}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("calc", "metrics", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// DecodeMetricsResponse returns a decoder for responses returned by the calc
...
func DecodeMetricsResponse(decoder func(*http.Response) goahttp.Decoder, restoreBody bool) func(*http.Response) (interface{}, error) {
	return func(resp *http.Response) (interface{}, error) {
		...
		case http.StatusOK:
			...
			err = decoder(resp).Decode(&body)
			if err != nil {
				return nil, goahttp.ErrDecodingError("calc", "metrics", err)
			}
			return body, nil
		default:
			body, _ := ioutil.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("calc", "metrics", resp.StatusCode, string(body))
		}
	}
}
